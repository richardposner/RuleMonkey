...
`harness/generate_basicmodels_refs.py`. Five of the original 36 models
are excluded — three for missing-feature reasons and two for pinning
NFsim-specific quirks that contradict BNG2.pl strict semantics:
  ...
  r36       — `-gml auto` regression test (NFsim issue #17 fallback;
              no RM equivalent)
  r33       — NFsim issue #22 ("occupied-site bond error": NFsim
              fires the rule even when AddBond would conflict, leaving
              B unbonded; BNG2.pl correctly limits firings to the free
              B pool, which is what RM does)
  r35       — NFsim issue #14 (rule RHS uses `.` to keep products in
              the same complex but NFsim ignores the constraint and
              splits; BNG2.pl generate_network refuses to emit the
              reaction because the result wouldn't be one complex,
              and RM matches BNG2.pl)

The remaining 31 are testable. Most are small fast models — full suite
