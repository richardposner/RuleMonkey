...
class TestNFSimFile(ParametrizedTestCase):
    ...
    def BNGtrajectoryGeneration(self, outputDirectory, fileNumber):
        bngFileName = os.path.join(outputDirectory, f"v{fileNumber}.bngl")
        with open(os.devnull, "w") as fnull:
            subprocess.check_call(
                ["perl", bngPath, "-outdir", outputDirectory, "-log", bngFileName], stdout=fnull
            )
        ...
        with open(os.devnull, "w") as fnull:
            subprocess.check_call(
                [
                    nfsimPath,
                    "-xml",
                    os.path.join(outputDirectory, f"v{fileNumber}.xml"),
                    "-o",
                    os.path.join(outputDirectory, f"v{fileNumber}_nf.gdat"),
                ]
                + runOptions,
                stdout=fnull,
            )
    ...
    def loadConfigurationFile(self, outputDirectory, fileNumber):
        with open(os.path.join(outputDirectory, "r{0}.txt").format(fileNumber)) as f:
            return f.readlines()
    ...
    def test_nfsim(self):
        ...
        if modelName.startswith("NFSIM ONLY"):
            ...
            nfh, nf = loadResults(
                os.path.join(self.param["odir"], "v{0}_nf.gdat".format(self.param["num"])), " "
            )
            ...
            for index in range(self.param["iterations"]):
                ...
                odeh, ode = loadResults(
                    os.path.join(self.param["odir"], "v{0}_ode.gdat".format(self.param["num"])), " "
                )
                ssah, ssa = loadResults(
                    os.path.join(self.param["odir"], "v{0}_ssa.gdat".format(self.param["num"])), " "
                )
                nfh, nf = loadResults(
                    os.path.join(self.param["odir"], "v{0}_nf.gdat".format(self.param["num"])), " "
                )
                ...
                if bad.size > 0:
                    print(
                        f"Sir, the observables {bad + 1} did not pass at seed={seed}. Trying again"
                    )
                else:
                    print("Check passed.")
                    break
...
class TestIssueRegressions(unittest.TestCase):
    ...
    def _bng_generate(self, outputDirectory, fileNumber):
        xmlFileName = os.path.join(outputDirectory, f"v{fileNumber}.xml")
    ...
    def test_issue49_species_observable_auto_enable_no_crash(self):
        ...
        self.assertTrue(
            np.isfinite(nf).all(), "Issue #49 failed: NFsim output contains non-finite values"
        )
        ...
        self.assertTrue(
            np.all(nf[:, dimerIdx] >= 0), "Issue #49 failed: Obs_Dimer has negative values"
        )
    ...
    for index in tests:
        suite.addTest(
            ParametrizedTestCase.parametrize(
                TestNFSimFile, param={"num": index, "odir": mfolder, "iterations": nIterations}
            )
        )
